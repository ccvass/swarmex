...
## 19. Ingress with Traefik-XPX

Swarmex uses [Traefik-XPX](https://github.com/ccvass/traefik-xpx) as its ingress controller — a hardened Traefik fork with AI Gateway, MCP Gateway, and a full CRUD dashboard.

### Prerequisites

Create shared mount directories on all manager nodes:

```bash
mkdir -p /mnt/traefik/{certs,logs,fileprovider}
```

Required files in the mount:

| File | Purpose |
|:---|:---|
| `/mnt/traefik/traefik-xp.yml` | Main Traefik config (entrypoints, resolvers, providers) |
| `/mnt/traefik/users.json` | Dashboard auth users (bcrypt hashed passwords) |
| `/mnt/traefik/fileprovider/dynamic.yml` | Dynamic config (TLS options, middleware chains) |
| `/mnt/traefik/certs/acme.json` | Auto-generated by Let's Encrypt (do not create manually) |

### Deploy

```bash
CF_API_EMAIL=admin@example.com CF_API_KEY=your-cloudflare-key \
  docker stack deploy -c stacks/ingress.yml traefik
```

### Configuration

The stack uses `--configFile=/etc/traefik/traefik.yml` which is mounted from your shared storage. This file controls all Traefik behavior. Example:

```yaml
# /mnt/traefik/traefik-xp.yml
entryPoints:
  http:
    address: ":80"
  https:
...
providers:
  ...
  file:
    filename: "/fileprovider/dynamic.yml"
    watch: true
...
api:
  dashboard: true
  valkeyUrl: "valkey://valkey:6379"  # optional, for distributed state
...
The ingress stack defines these labels on the Traefik service so legacy services continue working:

- `traefik.constraint-label=traefik-public` — services must have this label to be discovered
- `traefik.http.middlewares.https-redirect.redirectscheme.scheme=https` — HTTP→HTTPS redirect middleware

Any service using these labels from before Swarmex will work without changes:
...
Cloudflare DNS challenge supports wildcard certificates. Add A records pointing to any node IP:

```
app.domain.com    → <node-ip>
*.domain.com      → <node-ip>
```

Traefik automatically requests certificates for any `Host()` rule via Cloudflare. No configuration changes needed per domain.

### High Availability

Traefik runs on all manager nodes (global mode with manager constraint). Docker Swarm's routing mesh distributes traffic on ports 80/443 across all nodes. If a manager dies, traffic routes to surviving managers automatically.

For DNS failover, add multiple A records in Cloudflare — one per node IP. Cloudflare round-robins between healthy IPs.
